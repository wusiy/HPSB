...
//	}
    ...
    public int getValue(final int flag) {
        ...
        if (camera != null) {
            ...
            else if(flag == UVCCamera.PU_CONTRAST_AUTO){
                return camera.getAutoContrast() ? 1 : 0;
            } else if(flag == UVCCamera.CTRL_FOCUS_AUTO){
                return camera.getAutoFocus() ? 1 : 0;
            } else if(flag == UVCCamera.PU_HUE_AUTO){
                return camera.getAutoHue() ? 1 : 0;
            }
    ...
    public int setValue(final int flag, final int value) {
        ...
        if (camera != null) {
            if (flag == UVCCamera.PU_BRIGHTNESS) {
                ...
                camera.setAutoContrast(value == 1);
                return camera.getAutoContrast() ? 1 : 0;
            } else if(flag == UVCCamera.CTRL_FOCUS_AUTO){
                camera.setAutoFocus(value == 1);
                return camera.getAutoFocus() ? 1 : 0;
            } else if(flag == UVCCamera.PU_HUE_AUTO){
                camera.setAutoHue(value == 1);
                return camera.getAutoHue() ? 1 : 0;
            }

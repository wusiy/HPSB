...
public class StrategyChooseAdapter extends BaseAdapter implements View.OnClickListener {
    ...
    public void onClick(View view) {
        ...
        if (strategyCode == GdNaviUtils.PRIORITY_HIGHSPEED) {
            for (StrategyStateBean bean : mStrategys) {
                if (bean.getStrategyCode() == GdNaviUtils.AVOID_COST || bean.getStrategyCode() == GdNaviUtils.AVOID_HIGHSPEED) {
                    bean.setOpen(false);
                }
            }
        }

        if (strategyCode == GdNaviUtils.AVOID_COST || strategyCode == GdNaviUtils.AVOID_HIGHSPEED) {
            for (StrategyStateBean bean : mStrategys) {
